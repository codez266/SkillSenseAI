...
def test_create_interview_first_time(app, client):
    """Test creating an interview for the first time."""
    ...
    assert student.student_level == None
    ...
    assert interview.interview_problem_id is None
    assert interview.interview_policy is None
...
def test_create_interview_existing_session(app, client):
    ...
    response1 = client.get('/api/interview')
    assert response1.status_code == 201
    data1 = response1.get_json()

    # Second request should return same IDs
    response2 = client.get('/api/interview')
